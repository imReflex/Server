...
public class Sprite extends DrawingArea {
	...
	final void rotate(int degree) {
		//if (myWidth != maxWidth || myHeight != maxHeight) {
			int xOffset = degree;
			if (xOffset > drawOffsetX) {
				xOffset = drawOffsetX;
			}
			int xOffset2 = degree;
			if (xOffset2 + drawOffsetX + myWidth > maxWidth) {
				xOffset2 = maxWidth - drawOffsetX - myWidth;
			}
			int yOffset = degree;
			if (yOffset > drawOffsetY) {
				yOffset = drawOffsetY;
			}
			int yOffset2 = degree;
			if (yOffset2 + drawOffsetY + myHeight > maxHeight) {
				yOffset2 = maxHeight - drawOffsetY - myHeight;
			}
			int newWidth = myWidth + xOffset + xOffset2;
			int newHeight = myHeight + yOffset + yOffset2;
			int[] des = new int[newWidth * newHeight];
			for (int i_57_ = 0; i_57_ < myHeight; i_57_++) {
				for (int i_58_ = 0; i_58_ < myWidth; i_58_++)
					des[(i_57_ + yOffset) * newWidth + (i_58_ + xOffset)] = myPixels[i_57_ * myWidth + i_58_];
			}
			myPixels = des;
			myWidth = newWidth;
			myHeight = newHeight;
	...
	public Sprite(int[] pixels, int width, int height) {
		...
		drawOffsetX = 0;
		drawOffsetY = 0;
		myPixels = new int[pixels.length];

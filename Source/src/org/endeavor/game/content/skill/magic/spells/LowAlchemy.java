	...
	public boolean execute(Player player) {
		...
		if (!player.getInventory().hasSpaceFor(coins)) {
			player.getClient().queueOutgoingPacket(
					new SendMessage("You do not have enough inventory space to cast High alchemy."));
			return false;
		}
		...
		player.getUpdateFlags().sendGraphic(new Graphic(112, 0, false));

		player.getClient().queueOutgoingPacket(new SendOpenTab(PlayerConstants.MAGIC_TAB));

		player.getSkill().lock(4);

		player.getClient().queueOutgoingPacket(new SendSound(224, 0, 0));

...
public class ConstructionData {
	...
	public static enum Portals
	{
		...
		YANILLE(6, new Location(ConstructionConstants.YANILLE.getX(), ConstructionConstants.YANILLE.getY()), 58,
				new int[][] {{563, 200}, {557, 200}}, new int[] {13620, 13627, 13634}),
		KHARYLL(7, new Location(ConstructionConstants.KHARYRLL_X, ConstructionConstants.KHARYRLL_Y), 66,
			...
			if(requiredItems == null)
			{
				...
				while(it.hasNext())
				{
					...
					if(portal.getRoomX() == myTiles[0]-1
							...
							&& portal.getRoomZ() == (p.getFields().inDungeon() ? 4 : p.getLocation().getZ())
				...
				if(!found)
				{
					...
					p.getPacketSender().sendObjectsRemoval(myTiles[0]-1, myTiles[1]-1, p.getFields().inDungeon() ? 4 : p.getLocation().getZ());
					Construction.placeAllFurniture(p, myTiles[0]-1, myTiles[1]-1, p.getFields().inDungeon() ? 4 : p.getLocation().getZ());
					p.getPacketSender().sendInterfaceRemoval();
					return null;
				}
			}
		...
		public void build(Player p)
		{
			...
			if(!found)
			{
				...
				portal.setRoomZ(p.getFields().inDungeon() ? 4 : p.getLocation().getZ());
				portal.setType(type);
	...
	public static int getXOffsetForObjectId(int objectId, int hsId, int rotation) {
		...
		if (hs == null)
			return 0;
		ObjectDef objectDef = ObjectDef.getObjectDef(objectId);
